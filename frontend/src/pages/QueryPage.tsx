...
export default function QueryPage() {
  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="flex gap-1 mb-4">
        {modes.map(m => (
          <button key={m.k} onClick={() => setMode(m.k)}
            className={"px-4 py-1.5 rounded-lg text-xs font-semibold transition-colors " + (mode === m.k ? "bg-cyan-600 text-white" : "bg-gray-800 text-gray-400 hover:bg-gray-700")}>{m.l}</button>
        ))}
      </div>
      ...
      {sse.error && <div className="text-red-400 text-sm mb-4">Error: {sse.error}</div>}
      {cur && (
        <div className="bg-gray-900 rounded-xl p-6 mb-6">
          <div className="prose prose-invert prose-sm max-w-none">
            <ReactMarkdown remarkPlugins={[remarkGfm]}>{cur}</ReactMarkdown>
          </div>
          {loading && <span className="inline-block w-2 h-4 bg-cyan-400 animate-pulse ml-1 align-middle" />}
        </div>
      )}
      {curSrc.length > 0 && (
        <details className="bg-gray-900/50 rounded-xl p-4 mb-6">
          <summary className="cursor-pointer text-sm font-medium text-gray-400">Sources ({curSrc.length})</summary>
          <div className="mt-3 space-y-2">
            {curSrc.map((s: any, i: number) => (
              <div key={i} className="bg-gray-800 rounded-lg p-3 text-xs">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-cyan-400 font-medium">{s.file}</span>
                  <span className="text-gray-500">#{s.chunk_index}</span>
                  {s.score != null && <span className="text-green-400 ml-auto">{(s.score * 100).toFixed(0)}%</span>}
                </div>
                <p className="text-gray-400 line-clamp-3">{s.text?.slice(0, 200)}</p>
              </div>
            ))}
          </div>
        </details>
        ...
        <div className="space-y-4">
          ...
          {hist.slice(0, 10).map((h, i) => (
            <details key={i} className="bg-gray-900 rounded-xl p-4">
              <summary className="cursor-pointer text-sm font-medium text-gray-200 flex items-center gap-2">
                <span className="text-[10px] uppercase px-1.5 py-0.5 rounded bg-gray-700 text-gray-400">{h.mode}</span>
                {h.q}
              </summary>
